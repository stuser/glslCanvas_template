...
void main() {
    ...
    float info=texture2D(u_texBase,uv).g;
	
    //vec3 col=vec3(texh(uv*5.0, info+0.3-breathing*0.3));
	
	//材質改成圓型,生成速度減慢
	uv.x *= u_xBrickAmount/10.0; //由UI介面控制(X-axis)
    uv.y *= u_yBrickAmount/10.0; //由UI介面控制(Y-axis)
	vec3 col=vec3(texh2(uv, info+0.3-breathing*0.15));
